...
export async function readJson<T extends z.ZodType>(
  ...
  | {
      ok: true;
      data: z.infer<T>;
      present: <D extends object>(parsed: D) => Partial<D>;
    }
...
> {
  ...
  if (length > MAX_BODY_BYTES) {
    return {
      ok: false,
      response: fail("too-large", "That request body is too large.", 413),
    };
  }
...
export function route<T extends Response = NextResponse>(
  ...
  return handler().catch((err: unknown) => {
    ...
    if (err instanceof AIError)
      return fail(err.code, err.message, err.code === "no-key" ? 400 : 502);
