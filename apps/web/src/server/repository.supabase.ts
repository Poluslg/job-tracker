...
export class SupabaseRepository implements Repository {
  ...
  async getUserData(userId: string): Promise<UserData> {
    ...
    if (failure?.error) {
      throw new Error(
        `Could not load your workspace: ${failure.error.message}`,
      );
    }
  ...
  private async persistSettings(
    ...
    if (blobChanged) {
      ...
      if (error)
        throw new Error(`Could not save your settings: ${error.message}`);
    }
    ...
    if (after.ai.apiKey !== before.ai.apiKey) {
      ...
      if (error)
        throw new Error(`Could not save your API key: ${error.message}`);
    }
  }
