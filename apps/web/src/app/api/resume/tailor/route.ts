...
export async function POST(request: Request) {
  return route(async () => {
    ...
    const analysis =
      (body.analysisId
        ? data.analyses.find((a) => a.id === body.analysisId)
        : null) ?? data.analyses.filter((a) => a.jobId === job.id)[0];
    if (!analysis)
      return fail(
        "no-analysis",
        "Analyze this job before tailoring your resume.",
        400,
      );
    ...
    if (!settings.demoMode && !isConfigured(aiConfig)) {
      return fail(
        "no-key",
        "Add an AI provider key in Settings to use resume tailoring.",
        400,
      );
    }
