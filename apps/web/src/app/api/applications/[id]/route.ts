...
export async function GET(_request: Request, { params }: Params) {
  return route(async () => {
    ...
    if (!application)
      return fail("not-found", "That application does not exist.", 404);

    return ok({
      application,
      job: data.jobs.find((j) => j.id === application.jobId) ?? null,
      analysis:
        data.analyses.find((a) => a.id === application.analysisId) ?? null,
      coverLetter:
        data.coverLetters.find((c) => c.id === application.coverLetterId) ??
        null,
      interviewPrep:
