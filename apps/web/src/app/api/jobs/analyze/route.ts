...
export async function POST(request: Request) {
  return route(async () => {
    ...
    if (!body.job.description || body.job.description.trim().length < 100) {
      return fail(
        "no-description",
        "The job description is missing or too short to analyze.",
        422,
      );
    }
    ...
    const ai = new CareerAI({
      provider: createProvider(
        useAI ? aiConfig : { ...aiConfig, provider: "mock" },
      ),
      settings,
    });
    ...
    await mutateUserData((d) => {
      const existing = d.jobs.findIndex(
