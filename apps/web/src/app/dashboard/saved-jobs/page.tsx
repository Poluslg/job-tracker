...
export default async function SavedJobsPage() {
  ...
  for (const analysis of data.analyses) {
    if (!scoreByJob.has(analysis.jobId))
      scoreByJob.set(analysis.jobId, analysis.score.overall);
  }
              ...
              <Card key={job.id}>
                <CardBody className="flex h-full flex-col">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {score !== undefined && (
                      <Badge tone={bandFor(score).tone}>{score}</Badge>
                    )}
                  </div>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-1.5">
                    ...
                    {job.arrangement !== "unknown" && (
                      <Badge size="sm">{job.arrangement}</Badge>
                    )}
                    {job.platform !== "generic" && (
                      <Badge size="sm">{job.platform}</Badge>
                    )}
                  </div>
