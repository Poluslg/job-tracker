    ...
    <div className="mt-4 grid gap-4 lg:grid-cols-2">
      ...
      <Card>
        <CardBody>
          ...
          <p className="mt-0.5 mb-3 text-[11px] text-fg-subtle">
            Share of applications in each score band that advanced past
            submission. Descriptive only — no causal claim.
          </p>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={analytics.scoreVsOutcome}
                margin={{ top: 4, right: 4, bottom: 0, left: -20 }}
              >
                <CartesianGrid stroke={COLORS.grid} vertical={false} />
                ...
                <Tooltip
                  ...
                  formatter={(value, _name, item) => {
                    const row = (item as { payload?: unknown }).payload as
                      AnalyticsResponse["scoreVsOutcome"][number] | undefined;
                    return row
                      ? [
                          `${value}% (${row.advanced} of ${row.total})`,
                          "Advanced",
                        ]
                      : [`${value}%`, "Advanced"];
                  }}
