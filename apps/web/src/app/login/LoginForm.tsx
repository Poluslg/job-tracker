...
export function LoginForm() {
  ...
  const [error, setError] = useState<string | null>(
    ERROR_COPY[params.get("error") ?? ""] ?? null,
  );
    ...
    return (
      <div className="space-y-4">
        <Alert tone="warn" title="Google sign-in is not configured">
          ...
          <code>apps/web/.env.local</code>, then enable the Google provider in
          your Supabase project. See <code>docs/setup-supabase.md</code>.
        </Alert>
        <Button
          block
          variant="outline"
        ...
        <p className="text-center text-xs text-fg-subtle">
          Development only — uses the local JSON store and a fixed local
          identity.
        </p>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-center text-xs leading-relaxed text-fg-subtle">
        Google is used only to sign you in. The extension never posts anything
        on your behalf, and this app does not request access to your Gmail,
        Drive or contacts.
      </p>
    </div>
  );
