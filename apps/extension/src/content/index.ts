...
import { MIN_DETECTION_CONFIDENCE, extractFromSelection, extractJobFromDocument, urlLooksLikeJob } from '@job-ai/core';
import {
...
function detect(): JobExtractionResult {
  try {
    ...
    lastResult = {
      ...
      reason: err instanceof Error ? `Extraction failed: ${err.message}` : 'Extraction failed.',
    };
  }
  return lastResult;
}
...
function openPopupHint(): void {
  ...
  showToast('Click the AI Career Copilot icon in your toolbar to see your match.');
}
...
function updateFab(result: JobExtractionResult): void {
  ...
  if (result.ok) {
    renderFab({ label: 'Analyze this job', detected: true, onClick: openPopupHint });
      ...
      onClick: () =>
        startManualSelection(
          (text) => {
            ...
            void chrome.runtime.sendMessage({ type: 'JOB_DETECTED', payload: lastResult }).catch(() => {});
...
async function run(): Promise<void> {
  ...
  updateFab(result);
  if (result.ok) {
    void chrome.runtime.sendMessage({ type: 'JOB_DETECTED', payload: result }).catch(() => {});
  }
}
...
chrome.runtime.onMessage.addListener((message: ExtMessage, _sender, sendResponse) => {
  switch (message.type) {
    case 'EXTRACT_JOB': {
      const result = message.payload?.force || !lastResult ? detect() : lastResult;
...
setTimeout(() => {
  if (lastResult && lastResult.confidence < MIN_DETECTION_CONFIDENCE) void run();
}, 2500);
