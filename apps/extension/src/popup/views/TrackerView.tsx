...
export function TrackerView({
  ...
  const load = useCallback(async () => {
    try {
      ...
      onError(err instanceof MessageError ? err.message : 'Could not load your tracker.');
      setApplications([]);
    }
  }, [onError]);
  ...
  const updateStatus = async (id: string, status: ApplicationStatus) => {
    setBusyId(id);
    try {
      const result = await send({ type: 'UPDATE_APPLICATION', payload: { id, patch: { status } } });
      setApplications((prev) => prev?.map((a) => (a.id === id ? result.application : a)) ?? null);
      onDone(`Moved to ${STATUS_LABELS[status]}.`);
    } catch (err) {
      onError(err instanceof MessageError ? err.message : 'Could not update that application.');
    } finally {
      setBusyId(null);
    }
  };
    ...
    <div className="p-3">
      <div className="mb-3 flex items-center justify-between gap-2">
        ...
        <div className="flex gap-1">
          <Button size="sm" variant="ghost" loading={exporting} onClick={() => void exportTracker('csv')}>
            <Download className="h-3.5 w-3.5" /> CSV
          </Button>
          <Button size="sm" variant="ghost" loading={exporting} onClick={() => void exportTracker('xlsx')}>
      ...
      <ul className="space-y-2">
        {applications.map((app) => (
          <li key={app.id} className="rounded-lg border border-border bg-surface p-2.5">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="truncate text-xs font-medium text-fg">{app.title || 'Untitled role'}</p>
                <p className="truncate text-[11px] text-fg-muted">{app.company}</p>
              </div>
              <div className="flex shrink-0 items-center gap-1.5">
                {app.matchScore !== null && (
                  <span className="text-[11px] tabular-nums text-fg-muted">{app.matchScore}</span>
                )}
            ...
            {app.resumeVersionName && (
              <p className="mt-1.5 text-[10px] text-fg-subtle">Resume: {app.resumeVersionName}</p>
            )}
          </li>
        ))}
      </ul>
