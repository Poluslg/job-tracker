...
export function TailorPanel({
    ...
    try {
      const result = await send({
        ...
        payload: {
          ...
          acceptedIds: analysis.recommendations
            .filter((r) => !r.needsUserConfirmation)
            .map((r) => r.id),
          versionName,
        },
      });
      ...
      setDecisions(
        Object.fromEntries(
          result.changes.map((c) => [c.id, "pending" as Decision]),
        ),
      );
    } catch (err) {
      setError(
  ...
  const download = async (format: "pdf" | "docx") => {
    ...
    const safeName =
      versionName.replace(/[^\w -]/g, "").trim() || "tailored-resume";
    if (format === "docx") {
      await downloadBytes(
        buildDocx(resumeTextToBlocks(text)),
        `${safeName}.docx`,
        DOCX_MIME_TYPE,
      );
    } else {
      await downloadBytes(
        buildPdf(resumeTextToPdfBlocks(text), versionName),
        `${safeName}.pdf`,
        PDF_MIME_TYPE,
      );
    }
    ...
    return (
      <div className="space-y-3">
        <p className="text-xs text-fg-muted">
          Rewriting your resume for this role…
        </p>
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-xs leading-relaxed text-fg-muted">
            Suggestions are drawn only from what your resume already says.
            Nothing is invented, and you review every change before anything is
            downloaded.
          </p>
        </div>
  ...
  return (
    <div className="space-y-3">
      ...
      {changes.length === 0 && (
        <Alert tone="neutral">
          No changes were proposed. Your resume already reflects what this
          posting asks for.
        </Alert>
      )}
            ...
            <CardBody className="space-y-2">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() =>
                    setDecisions((p) => ({ ...p, [change.id]: "accepted" }))
                  }
                >
                ...
                <Button
                  ...
                  onClick={() =>
                    setDecisions((p) => ({ ...p, [change.id]: "rejected" }))
                  }
                >
      ...
      <Alert tone="warn">
        Read every accepted change before you send this resume. If a rewrite
        implies something you cannot speak to in an interview, edit it or reject
        it.
      </Alert>
