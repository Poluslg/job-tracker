...
export async function send<T extends ExtMessage["type"]>(
  ...
  try {
    response = (await chrome.runtime.sendMessage(message)) as ExtResponse<
      ExtResponseMap[T]
    >;
  } catch (err) {
  ...
  if (!response)
    throw new MessageError("empty", "The background service did not respond.");
  if (!response.ok)
    throw new MessageError(response.error.code, response.error.message);
...
export function isRestrictedUrl(url: string | undefined): boolean {
  if (!url) return true;
  return (
    /^(chrome|edge|about|devtools|view-source|chrome-extension|moz-extension):/i.test(
      url,
    ) ||
