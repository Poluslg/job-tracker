...
test('settings parse from an empty object and never ship a key', () => {
  ...
  assert.equal(settings.privacy.redactContactInfo, true, 'redaction is on by default');
  assert.equal(settings.privacy.shareAnonymousUsage, false, 'telemetry is opt-in');
  assert.equal(settings.privacy.syncEnabled, false, 'sync is opt-in');
});

test('default scoring weights sum to 1', () => {
  const total = Object.values(DEFAULT_SETTINGS.scoring).reduce((a, b) => a + b, 0);
...
test('a partial patch schema re-applies defaults for absent fields', () => {
  ...
  assert.equal(parsed.company, '', 'absent fields come back at their default, not undefined');
...
test('an application requires a discovery timestamp and defaults to saved', () => {
  ...
  const app = Application.parse({ id: 'a', jobId: 'j', discoveredAt: now, createdAt: now, updatedAt: now });
