import { z } from 'zod';
import { Confidence, ExtractionSource, Id, IsoDate, Timestamped } from './common.ts';
...
export const WorkArrangement = z.enum(['onsite', 'hybrid', 'remote', 'unknown']);
...
export const SalaryInfo = z.object({
  ...
  period: z.enum(['hour', 'day', 'month', 'year', 'unknown']).default('unknown'),
  raw: z.string().default(''),
});
export type SalaryInfo = z.infer<typeof SalaryInfo>;

export const RequirementKind = z.enum(['must-have', 'nice-to-have', 'responsibility', 'signal']);
