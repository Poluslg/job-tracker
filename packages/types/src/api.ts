import { z } from 'zod';
import { Application, CoverLetter, CoverLetterTone, InterviewPrep } from './application.ts';
...
export function apiErr(code: string, message: string, fields?: Record<string, string>): ApiResult<never> {
...
export const AnalyzeJobResponse = z.object({ job: JobPosting, analysis: JobAnalysis });
...
export type TailorChange = z.infer<typeof TailorResumeResponse>['changes'][number];
...
export const AnalyticsResponse = z.object({
  ...
  funnel: z.array(z.object({ stage: z.string(), label: z.string(), count: z.number() })),
  weekly: z.array(
    z.object({
      week: z.string(),
      applications: z.number(),
  ...
  scoreVsOutcome: z.array(
    z.object({ bucket: z.string(), total: z.number(), advanced: z.number(), rate: z.number() }),
  ),
  topCompanies: z.array(z.object({ name: z.string(), count: z.number() })),
  topTitles: z.array(z.object({ name: z.string(), count: z.number() })),
  skillGaps: z.array(
    z.object({ skill: z.string(), jobsRequiring: z.number(), share: z.number() }),
  ),
  highestMatches: z.array(
    z.object({ id: Id, company: z.string(), title: z.string(), score: z.number() }),
  ),
});
