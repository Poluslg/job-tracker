...
export const AIProviderId = z.enum(['openai', 'anthropic', 'gemini', 'openrouter', 'mock']);
...
export class AIError extends Error {
  ...
  constructor(code: AIErrorCode, message: string, retryable = false, status?: number) {
    super(message);
    this.name = 'AIError';
    this.code = code;
...
export const AI_ERROR_MESSAGES: Record<AIErrorCode, string> = {
  'no-key': 'No API key configured. Add one in Settings, or continue with local-only analysis.',
  'invalid-key': 'Your API key was rejected by the provider. Check it in Settings.',
  'rate-limited': 'The provider rate-limited this request. Wait a moment and try again.',
  'quota-exceeded': 'Your provider account is out of quota or credit.',
  unavailable: 'The AI provider is currently unavailable. Local analysis is still available.',
  network: 'Network request failed. Check your connection and try again.',
  timeout: 'The request took too long and was cancelled.',
  'too-large': 'This job description or resume is too large to send. It has been truncated — try again.',
  'invalid-response': 'The model returned an unexpected format. You can retry or use local analysis.',
  blocked: 'The provider blocked this request. Try a different model or provider.',
