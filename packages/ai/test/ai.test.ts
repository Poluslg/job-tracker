...
test('repairJson recovers objects from fenced and chatty responses', () => {
  ...
  assert.equal(repairJson('Sure! Here you go:\n{"a":1}\nHope that helps.'), '{"a":1}');
  assert.equal(repairJson('{"a":"has } brace","b":2}'), '{"a":"has } brace","b":2}');
...
test('every prompt carries the anti-fabrication and untrusted-content rules', () => {
  ...
  assert.match(matchInsightsPrompt.system, /do NOT calculate or revise the score/i);
});
...
test('a job description containing an injection is still analyzed as data', async () => {
  ...
  const { analysis, aiError } = await ai.analyzeJob(resume, hostile, { useAI: true });
...
test('analysis degrades to local when the provider fails', async () => {
  ...
  const { analysis, aiError } = await ai.analyzeJob(resume, job, { useAI: true });

  assert.ok(aiError, 'the failure should be surfaced, not swallowed');
  assert.equal(analysis.mode, 'local');
  assert.ok(analysis.score.overall > 0, 'the user still gets a usable analysis');
...
test('runPrompt validates against the schema and retries invalid shapes once', async () => {
  ...
  const flaky = {
    ...
    complete: async () => {
      calls++;
      return {
        text: calls === 1 ? 'not json at all' : JSON.stringify({ concerns: [{ text: 'ok', severity: 'low' }] }),
        usage: { inputTokens: 0, outputTokens: 0, model: 'x', provider: 'mock' as const, latencyMs: 0 },
      };
    },
    testConnection: async () => ({ ok: true as const }),
  };
  ...
  const result = await runPrompt(flaky, matchInsightsPrompt, AIMatchInsights, {
    jobTitle: 'x', company: 'y', description: 'z', resumeText: 'r',
    score: 50, strongSkills: [], partialSkills: [], missingRequired: [],
    experienceNote: '', atsCoverage: 0,
  });
...
test('cover letter generation returns confirmation items', async () => {
  const ai = new CareerAI({ provider: new MockProvider(0), settings });
  const letter = await ai.generateCoverLetter(resume, job, { tone: 'professional' });
...
test('tailoring never mutates the original profile and skips unmatched changes', async () => {
  ...
  assert.ok(unverifiable.length > 0, 'unverifiable claims must be reported, not applied');

  const summaryChange = changes.find((c) => c.section === 'Summary')!;
  const before = resume.profile.summary;
  const { profile, applied, skipped } = applyTailorChanges(resume.profile, changes, [summaryChange.id]);

  assert.equal(applied, 1);
  assert.equal(skipped.length, 0);
  assert.equal(resume.profile.summary, before, 'the original profile must be untouched');
...
test('a change whose original text is not found is skipped, never guessed', () => {
  const result = applyTailorChanges(
    resume.profile,
    [{ id: 'c1', section: 'Experience', original: 'text that does not exist', suggested: 'new', reason: '', needsUserConfirmation: false }],
    ['c1'],
  );
...
test('provider registry reports configuration state without exposing keys', () => {
  assert.equal(isConfigured({ ...DEFAULT_SETTINGS.ai, provider: 'openai', apiKey: '' }), false);
  assert.equal(isConfigured({ ...DEFAULT_SETTINGS.ai, provider: 'openai', apiKey: 'sk-x' }), true);
  assert.equal(isConfigured({ ...DEFAULT_SETTINGS.ai, provider: 'mock', apiKey: '' }), true);

  const provider = createProvider({ ...DEFAULT_SETTINGS.ai, provider: 'anthropic', apiKey: 'sk-secret' });
