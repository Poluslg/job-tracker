...
type Draft = Partial2<JobPosting> & {
...
function visibleText(el: Element): string {
  ...
  for (const block of [
    ...clone.querySelectorAll("p,div,h1,h2,h3,h4,h5,h6,tr"),
  ]) {
...
function fromJsonLd(job: JsonLdJob): Draft {
  ...
  const set = <K extends keyof JobPosting>(
    key: K,
    value: JobPosting[K] | undefined,
    truthy = true,
  ) => {
  ...
  if (
    job.jobLocationType &&
    /telecommute|remote/i.test(String(job.jobLocationType))
  ) {
...
function normalizeEmploymentType(
  ...
  const v = lower(Array.isArray(input) ? (input[0] ?? "") : input).replace(
    /[_\s]/g,
    "-",
  );
...
function heuristicDescription(doc: Document): Candidate | null {
  const roots = [
    ...doc.querySelectorAll('main, article, section, div, [role="main"]'),
  ];
...
function guessSalary(text: string): SalaryInfo {
  ...
  if (!m)
    return { min: null, max: null, currency: "", period: "unknown", raw: "" };
  const raw = m[0].trim();
  ...
  const period = /hour|hr/i.test(raw)
    ? "hour"
    : /month|mo\b/i.test(raw)
      ? "month"
      : /year|yr|annum/i.test(raw)
        ? "year"
        : "unknown";
  return {
    min: nums[0] ?? null,
...
function guessArrangement(text: string): WorkArrangement {
  ...
  if (
    /\b(fully remote|remote[- ]first|100% remote|work from home|remote)\b/.test(
      t,
    )
  )
    return "remote";
...
export function extractJobFromDocument(
  ...
  if (!draft.description) {
    const semantic = doc.querySelector(
      '[itemprop="description"], article, main, [role="main"]',
    );
  ...
  if (!draft.location) {
    const locMeta = doc.querySelector(
      '[itemprop="jobLocation"], [class*="location" i]',
    );
  ...
  if (!draft.arrangement || draft.arrangement === "unknown") {
    draft.arrangement = guessArrangement(
  ...
  const primarySource: ExtractionSource =
    (draft.fieldSources["description"] as ExtractionSource | undefined) ??
    "heuristic";
...
export function fingerprintFor(
  ...
  return stableHash(
    `${lower(company)}|${lower(title)}|${lower(description).slice(0, 2000)}`,
  );
}
  ...
  return {
    ...
    reason:
      description.length > 100
        ? ""
        : "The selected text was too short to analyze.",
  };
}
