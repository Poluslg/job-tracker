...
const MUST_INLINE =
  /\b(must have|required|require[sd]?|you have|minimum of|at least|proven|demonstrated|strong (background|experience))\b/i;
const NICE_INLINE =
  /\b(nice to have|preferred|bonus|a plus|ideally|desirable|familiarity with|exposure to)\b/i;
const RESP_INLINE =
  /^(design|build|develop|implement|own|lead|drive|collaborate|partner|maintain|ship|deliver|support|mentor|write|create|improve|scale|monitor|manage|work with|contribute)\b/i;
...
export function extractYears(text: string): number | null {
  const digit = text.match(
    /(\d{1,2})\s*\+?\s*(?:-|to)?\s*(?:\d{1,2})?\s*\+?\s*(?:years?|yrs?)\b/i,
  );
  if (digit?.[1]) {
  ...
  const word = text.match(
    /\b(one|two|three|four|five|six|seven|eight|nine|ten)\s+(?:\+\s*)?years?\b/i,
  );
...
export function extractRequirements(description: string): JobRequirement[] {
  ...
  for (const raw of lines) {
    ...
    if (line.length <= 80) {
      const cue = SECTION_CUES.find((c) =>
        c.patterns.test(line.replace(/[:：]$/, "")),
      );
      if (cue) {
        section = cue.kind;
        continue;
...
export function requirementsByKind(
  reqs: JobRequirement[],
