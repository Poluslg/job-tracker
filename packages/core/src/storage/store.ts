...
export class DataStore {
  ...
  async saveResume(resume: Resume): Promise<Resume> {
    ...
    if (idx >= 0) all[idx] = next;
      ...
      all.push({
        ...next,
        isDefault: all.length === 0 ? true : next.isDefault,
      });
  ...
  async deleteResume(id: string): Promise<void> {
    await this.put(
      KEYS.resumes,
      (await this.getResumes()).filter((r) => r.id !== id),
    );
    await this.put(
      KEYS.versions,
      (await this.getVersions()).filter((v) => v.resumeId !== id),
    );
  }
  ...
  async deleteVersion(id: string): Promise<void> {
    await this.put(
      KEYS.versions,
      (await this.getVersions()).filter((v) => v.id !== id),
    );
  }
  ...
  async saveJob(job: JobPosting): Promise<JobPosting> {
    ...
    const existingIdx = all.findIndex(
      (j) =>
        j.id === job.id ||
        (job.fingerprint !== "" && j.fingerprint === job.fingerprint),
    );
    if (existingIdx >= 0) {
      const merged = {
        ...all[existingIdx]!,
  ...
  async deleteJob(id: string): Promise<void> {
    await this.put(
      KEYS.jobs,
      (await this.getJobs()).filter((j) => j.id !== id),
    );
  }
  ...
  async getLatestAnalysisForJob(jobId: string): Promise<JobAnalysis | null> {
    const list = (await this.getAnalyses(jobId)).sort((a, b) =>
  ...
  async getApplicationByJob(jobId: string): Promise<Application | null> {
    return (
      (await this.getApplications()).find((a) => a.jobId === jobId) ?? null
    );
  }
  ...
  async deleteApplication(id: string): Promise<void> {
    await this.put(
      KEYS.applications,
      (await this.getApplications()).filter((a) => a.id !== id),
    );
  }
  ...
  async deleteCoverLetter(id: string): Promise<void> {
    await this.put(
      KEYS.coverLetters,
      (await this.getCoverLetters()).filter((c) => c.id !== id),
    );
  }
  ...
  async clear(
    scope: "all" | "applications" | "resumes" | "ai-key",
  ): Promise<void> {
    switch (scope) {
      case "all": {
...
export function applicationFromJob(
  ...
  return {
    ...
    salary: job.salary?.raw || (job.salary ? formatSalary(job.salary) : ""),
