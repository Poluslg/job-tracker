...
import { buildResumeSkillIndex, compareSkills, coverageScore } from './skills.ts';
...
export interface ScoreInput {
  ...
  resumeText: string;
  job: Pick<JobPosting, 'title' | 'company' | 'description'> & Partial<JobPosting>;
...
export function normalizeWeights(weights: ScoringWeights): ScoringWeights {
  ...
  for (const [k, v] of Object.entries(weights) as Array<[keyof ScoringWeights, number]>) {
    out[k] = v / total;
  }
  return out;
}

export function computeAnalysis(input: ScoreInput): Omit<JobAnalysis, 'id' | 'jobId' | 'resumeId' | 'createdAt' | 'updatedAt'> {
  ...
  const requiredScore = requiredMatches.length ? coverageScore(requiredMatches) : 65;
  const preferredScore = preferredMatches.length ? coverageScore(preferredMatches) : 65;
  ...
  const components: ScoreComponent[] = [
    ...
    },
    {
      ...
      details: responsibilities.uncovered.slice(0, 5).map((r) => `Not evidenced: ${r.slice(0, 120)}`),
    },
    {
      key: 'keywords',
      ...
      details: ats.missing.filter((k) => k.important).slice(0, 8).map((k) => `Missing: ${k.keyword}`),
    },
    {
      key: 'education',
  ...
  const score: ScoreBreakdown = { overall, components, engineVersion: SCORING_ENGINE_VERSION };
  ...
  return {
    ...
    modelUsed: '',
    promptVersion: '',
    confidence: confidenceFor(description, input.resumeText, requirements.length),
  };
}
...
function confidenceFor(description: string, resumeText: string, reqCount: number): 'high' | 'medium' | 'low' {
...
export function toAnalysisRecord(
  ...
  const now = nowIso();
  return { id: createId('an'), jobId, resumeId, createdAt: now, updatedAt: now, ...partial };
}

export function scoreBand(score: number): { label: string; tone: 'strong' | 'good' | 'fair' | 'low' } {
