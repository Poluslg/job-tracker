...
const METRIC_PATTERN = /\d+\s*(%|percent|x\b|k\b|m\b|ms\b|users|requests|customers|hours|weeks|days)/i;

export function buildRecommendations(input: RecommendationInput): Recommendation[] {
  const recs: Recommendation[] = [];
  const push = (r: Omit<Recommendation, 'id'>) => recs.push({ id: createId('rec'), ...r });

  const partials = input.matches.filter((m) => m.quality === 'partial' && m.required);
  if (partials.length) {
    push({
  ...
  const missingRequired = input.matches.filter((m) => m.quality === 'missing' && m.required);
  ...
  const strongSkills = input.matches.filter((m) => m.quality === 'strong' && m.required).map((m) => m.skill);
  const topListed = input.profile.skills.slice(0, 8).map((s) => s.name.toLowerCase());
  const buried = strongSkills.filter((s) => !topListed.includes(s.toLowerCase()));
  if (buried.length) {
    push({
      kind: 'reorder-skills',
  ...
  const bullets = input.profile.experience.flatMap((e) => [...e.responsibilities, ...e.achievements]);
  ...
  if (input.uncoveredResponsibilities.length) {
    push({
      ...
      detail:
        `These day-to-day responsibilities are not reflected in your resume: ${input.uncoveredResponsibilities
          .slice(0, 3)
          .map((r) => `"${r.slice(0, 90)}"`)
          .join('; ')}. If your current work covers any of them, rewrite an existing bullet to say so plainly.`,
      priority: 'medium',
      needsUserConfirmation: true,
    });
  }
  ...
  const irrelevant = input.profile.skills.filter(
    (s) => !input.matches.some((m) => m.skill.toLowerCase() === s.name.toLowerCase()),
  );
