import type { EducationAlignment, ExperienceAlignment, JobRequirement, ResumeProfile } from '@job-ai/types';
...
export function parseResumeDate(input: string): Date | null {
  ...
  if (numeric?.[1] && numeric[2]) return new Date(Number(numeric[2]), Number(numeric[1]) - 1, 1);
  const isoish = s.match(/(\d{4})[-/](\d{1,2})/);
  if (isoish?.[1] && isoish[2]) return new Date(Number(isoish[1]), Number(isoish[2]) - 1, 1);
...
export function totalExperienceYears(profile: ResumeProfile): number | null {
  ...
  for (const e of profile.experience) {
    ...
    if (!start) continue;
    const end = e.current ? new Date() : (parseResumeDate(e.endDate) ?? new Date());
...
export function analyzeExperience(
  ...
  const verdict = ratio >= 1.25 ? 'above' : ratio >= 1 ? 'meets' : ratio >= 0.75 ? 'near' : 'below';
  ...
  return { alignment: { resumeYears, requiredYears: required, verdict, note, relevantTitles }, score };
}
...
const DEGREE_LEVELS: Array<{ level: number; patterns: RegExp }> = [
  ...
  { level: 2, patterns: /\b(b\.?s\.?c?|bachelor'?s?|b\.?eng|b\.?tech|b\.?a\b)\b/i },
...
export function analyzeEducation(
  ...
  const equivalentAccepted = /\b(or equivalent|equivalent (practical )?experience|or relevant experience)\b/i.test(
    jobDescription,
  );
  ...
  const credentials = [
    ...profile.education.filter((e) => e.degree).map((e) => `${e.degree}${e.field ? `, ${e.field}` : ''}`),
