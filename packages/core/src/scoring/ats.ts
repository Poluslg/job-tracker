...
export function importantKeywords(
  description: string,
  limit = 40,
  ...
  const merged = new Map<
    string,
    { term: string; count: number; score: number }
  >();
  ...
  for (const cand of all) {
    const redundant = kept.some(
      (k) =>
        k.term.toLowerCase().includes(cand.term.toLowerCase()) &&
...
function titleAlignment(
  ...
  const jt = tokenize(jobTitle).filter(
    (t) =>
      !["senior", "junior", "staff", "lead", "principal", "ii", "iii"].includes(
        t,
      ),
  );
...
function detectAtsIssues(resumeText: string): string[] {
  ...
  if (resumeText.length < 800) {
    issues.push(
      "Your resume text is very short — parsers may have missed content in a complex layout.",
    );
  }
  if (/\t{2,}| {6,}/.test(resumeText)) {
    issues.push(
      "Detected wide column spacing. Multi-column layouts are often reordered by ATS parsers.",
    );
  }
  if (!/\b(experience|employment|work history)\b/i.test(resumeText)) {
    issues.push(
      'No clearly labelled "Experience" section was found. Standard section headings parse more reliably.',
    );
  }
  ...
  if (bulletLines < 3) {
    issues.push(
      "Few bullet points detected. Bulleted achievements parse and scan better than paragraphs.",
    );
  }
  return issues;
}
