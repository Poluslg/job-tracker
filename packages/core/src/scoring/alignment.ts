...
export function analyzeResponsibilities(
...
): { score: number; covered: string[]; uncovered: string[]; coverage: Record<string, MatchQuality> } {
  ...
  for (const r of resps) {
    ...
    const quality: MatchQuality = strength >= 60 ? 'strong' : strength >= 30 ? 'partial' : 'missing';
...
const DOMAINS: Record<string, string[]> = {
  Fintech: ['fintech', 'payments', 'banking', 'trading', 'ledger', 'kyc', 'compliance', 'lending'],
  Healthcare: ['healthcare', 'clinical', 'patient', 'hipaa', 'ehr', 'medical', 'telehealth'],
  'E-commerce': ['e-commerce', 'ecommerce', 'checkout', 'marketplace', 'catalog', 'fulfillment', 'retail'],
  'Developer Tools': ['developer tools', 'sdk', 'cli', 'api platform', 'devtools', 'open source'],
  Gaming: ['gaming', 'game', 'unity', 'unreal', 'multiplayer'],
  Education: ['education', 'edtech', 'learning', 'curriculum', 'students'],
  Security: ['security', 'threat', 'vulnerability', 'soc', 'siem', 'penetration'],
  'AI/ML': ['llm', 'generative ai', 'machine learning', 'model training', 'inference', 'embeddings'],
...
export function analyzeDomain(
  ...
  return { score: clamp(Math.round(30 + ratio * 70)), shared, jobDomains, note };
}
