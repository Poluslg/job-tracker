...
test("total experience merges overlapping roles", () => {
  ...
  assert.ok(
    years !== null && years > 5 && years < 20,
    `unexpected years: ${years}`,
  );
});
...
test("broader resume skill yields a PARTIAL match, not a strong one", () => {
  const index = buildResumeSkillIndex(
    {
      ...resume.profile,
      skills: [{ name: "AWS", category: "technical", years: null }],
    },
    "Worked with AWS.",
  );
  const match = matchSkill(
    ...
    "AWS Lambda",
    true,
    "Experience with AWS Lambda required.",
  );
...
test("more specific resume skill satisfies the broader requirement", () => {
  const index = buildResumeSkillIndex(
    {
      ...resume.profile,
      skills: [{ name: "React Native", category: "technical", years: null }],
    },
...
test("scoring is deterministic and explainable", () => {
  ...
  for (const c of a.score.components) {
    assert.ok(
      c.explanation.length > 10,
      `component ${c.key} must explain itself`,
    );
  ...
  assert.ok(
    a.score.overall > 45,
    `sample resume should match sample job well: ${a.score.overall}`,
  );
});
...
test("analysis surfaces required gaps and never fabricates", () => {
  ...
  assert.ok(
    risky.length > 0,
    "gap-filling advice must be flagged for confirmation",
  );
});
...
test("score bands never claim a hiring outcome", () => {
  for (const s of [10, 50, 70, 95]) {
    assert.doesNotMatch(
      scoreBand(s).label,
      /interview|hire|offer|chance|likel/i,
    );
  }
});
...
test("skill detection handles punctuation-heavy names", () => {
  ...
  for (const expected of ["C++", "C#", "Node.js", "Next.js"]) {
    assert.ok(
      ...
      `missing ${expected} in ${found.join(", ")}`,
    );
  }
});
...
test("compareSkills treats catalog skills outside requirements as preferred", () => {
  const index = buildResumeSkillIndex(resume.profile, resume.origin.rawText);
  const { matches } = compareSkills(
    index,
    extractRequirements(job.description),
    job.description,
  );
  assert.ok(
    matches.some((m) => m.required),
    "expected required matches",
  );
  assert.ok(
    matches.some((m) => !m.required),
    "expected preferred matches",
  );
});
...
test("data store round-trips applications and records status history", async () => {
  ...
  const app = await store.saveApplication(
    applicationFromJob(savedJob, { status: "saved" }),
  );
...
test("exports excluded the API key and produce valid files", async () => {
  ...
  await store.updateSettings({
    ai: { ...DEFAULT_SETTINGS.ai, apiKey: "sk-secret-value" },
  });
  const dump = JSON.stringify(await store.exportAll());
  assert.ok(
    !dump.includes("sk-secret-value"),
    "API keys must never appear in exports",
  );
});
...
test("analytics computes a funnel without claiming causation", () => {
  ...
  const analysis = toAnalysisRecord(
    computeAnalysis({
      profile: thinProfile,
      resumeText: "Alex Rivera. Wrote some HTML.",
      job,
    }),
    job.id,
    resume.id,
  ...
  assert.ok(
    ...
    "expected skill gaps from the analysis",
  );
});
