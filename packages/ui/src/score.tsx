  ...
  return (
    <div className="space-y-2">
      ...
      <p className="pt-1 text-[11px] leading-relaxed text-fg-subtle">
        Total: {score.overall} / 100 · scoring engine v{score.engineVersion}.{" "}
        {SCORE_DISCLAIMER}
      </p>
    </div>
  );
  ...
  const groups = [
    {
      ...
      hint: "Clearly present in both your resume and the posting.",
    },
    {
      key: "partial",
      title: "Partial matches",
      hint: "Your resume shows related experience, not the exact requirement.",
    },
    {
      key: "missing",
      title: "Not found in your resume",
      hint: "Only add these if you genuinely have the experience.",
    },
  ] as const;
    ...
    <div className="space-y-4">
      <Card>
        <CardBody className="space-y-2">
          ...
          <Progress
            value={ats.coverage}
            tone={bandFor(ats.coverage).tone}
            label="ATS keyword coverage"
          />
          <p className="text-[11px] text-fg-subtle">
            Share of the posting&rsquo;s highest-signal terms that appear
      ...
      {missingImportant.length > 0 && (
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-danger">
            Do not add a keyword unless you genuinely have that experience.
            Padding a resume with terms you cannot discuss will not survive an
            interview.
          </p>
          <div className="mt-2 flex flex-wrap gap-1.5">
            {missingImportant.slice(0, 20).map((k) => (
              <Tooltip
                key={k.keyword}
                content={`Appears ${k.inJob}× in the posting, 0× in your resume.`}
              >
        ...
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {ats.found.slice(0, 20).map((k) => (
              <Tooltip
                key={k.keyword}
                content={`${k.inJob}× in the posting, ${k.inResume}× in your resume.`}
              >
